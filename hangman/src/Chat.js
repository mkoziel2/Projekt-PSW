...
const Chat = ({audienceId, chat, started, players, yourId, game}) => {
    ...
    const msgGen = (msg) => {
        if (msg.target === 0) {
            ...
            if (msg.target === yourId) {
                return `*PV* P${msg.player}: ${msg.text}`
            } else {
                return `*PV(${msg.target})* P${msg.player}: ${msg.text}`
            }
